...
public class WonderBoard {
    ...
    private String scientist; // Magic Card Effect #3
        ...
        if(temp2 == null)
          {
            ...
            for( int a = 0; a < this.sourcesToCalculate.size(); a++)
            {
                    char tempAr[] = this.sourcesToCalculate.get(a).toCharArray();
                    Arrays.sort(tempAr);
                    String str = new String((tempAr));
                    temp3.add(str);
            }
            this.setSourcesToCalculate(temp3);
          }
            else
            {
